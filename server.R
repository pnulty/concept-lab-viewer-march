...
draw_network2D <- function(merged){
    ...
    vn <- visNetwork(nodes=td$nodes, edges=td$edges, height="100%", width="100%")%>%  visIgraphLayout(layout = input$`2d_layout`) %>%
        visOptions(highlightNearest = list(enabled = T, hover = T), 
                   nodesIdSelection = T) %>% visExport(name=paste(input$term, input$dataset,input$decade,input$measure_radio, input$thresh,input$rank,input$network_steps, input$prune_slide,input$conc,sep='-'))
}
  ...
  update_data <- reactive({
    ...
    if(input$dataset == 'ecco'){
      output$user_feedback <- renderText({'updating main data file... reading data.'})
      cur_file <- paste0('ecco_',input$decade,'_dist_',input$distance,'_cut_10_2.feather')
    }else if(input$dataset == 'reddit-libertarian'){
      cur_file <- 'reddit_lib_since_2015.feather'
    }
    else if(input$dataset == 'reddit-socialism'){
      cur_file <- 'reddit_soc_since_2015.feather'
    }
    else if(input$dataset == 'all3cities'){
      cur_file <- 'scottish_test_3.feather'
    }
    rels <- read_feather(paste0('../data/',cur_file))
    
  rels <- mutate(rels, score = get_score(input$measure_radio, Freq, e11))
  
    
    if(input$use_dict){
      rels <- filter(rels, focal %in% conc$Word)
      rels <- filter(rels, bound %in% conc$Word)
    }
  ...
  output$main_table <- renderDataTable({
    ...
    rels <- update_data()
    print(names(rels))
    t1 <- filter(rels, focal == input$term)
  }, searchDelay = 800)
